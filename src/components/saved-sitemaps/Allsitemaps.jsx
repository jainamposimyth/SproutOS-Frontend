...
const PageNode = ({ id, data }) => {
    ...
    // helper to determine if a section is global:
    const isSectionGlobal = (section) => {
        // 1) section-level flag from saved payload
        if (section && typeof section === 'object' && section.isGlobal) return true;

        // 2) per-node/global list provided on node.data.globalSections (strings)
        const name = getSectionName(section).toLowerCase().trim();
        const nodeGlobals = Array.isArray(data?.globalSections) ? data.globalSections : [];
        if (nodeGlobals.some(g => String(g).toLowerCase().trim() === name)) return true;

        // 3) top-level data.globalLabels (sometimes returned) — fallback
        const topGlobals = Array.isArray(data?.globalLabels) ? data.globalLabels : [];
        if (topGlobals.some(g => String(g).toLowerCase().trim() === name)) return true;
        ...
        <div className="relative bg-white border rounded-xl shadow-sm p-3 w-32">
            ...
            <div className="flex flex-col gap-2">
                {data.sections?.map((section, idx) => {
                    const global = isSectionGlobal(section);
                    return (
          ...
          ${global ? 'data-[state=open]:bg-sprout-color-success-lightest data-[state=open]:border-sprout-color-success' : 'data-[state=open]:bg-white'}
        `}
                            >
                                <AccordionTrigger
                                    className={
                                        "flex items-center justify-between w-full px-2 py-1.5 text-xs font-medium no-underline hover:no-underline " +
                                        (global
                                            ? "text-sprout-color-text-default bg-sprout-color-success-lightest border-sprout-color-success rounded-md"
                                ...
                                <AccordionContent
                                    className={
                                        "px-2 py-1 text-[10px] leading-snug " +
                                        (global
                                            ? "text-sprout-color-text-weakest bg-sprout-color-success-lightest border-b border-sprout-color-success"
