...
const Sections = () => {
  ...
  const imageElements = currentElements.filter(element => {
    ...
    return (
      tagName === 'img' ||
      className.includes('image') || className.includes('img') ||
      className.includes('photo') || className.includes('picture') ||
      id.includes('image') || id.includes('img') || id.includes('photo') ||
      src.includes('.jpg') || src.includes('.jpeg') ||
      src.includes('.png') || src.includes('.svg') ||
      src.includes('.gif') || src.includes('.webp') ||
      alt.includes('hero') || alt.includes('banner') ||
      alt.includes('image') || alt.includes('photo')
    );
  });
...
setAllImages(imageElements)
  ...
  const sectionElements = currentElements.filter(element => {
    ...
    return (
      ...
      tagName === 'p' || tagName === 'span' || tagName === 'button' ||
      tagName === 'a' || tagName === 'img' ||
      className.includes('hero') || className.includes('title') ||
      className.includes('heading') || className.includes('description') ||
      className.includes('subtitle') || className.includes('text') ||
      className.includes('content') || className.includes('caption') ||
      id.includes('hero') || id.includes('title') || id.includes('heading') ||
      id.includes('description') || text.includes('hero') || text.includes('title') ||
      text.includes('heading') || text.includes('description') || text.includes('subtitle') ||
      text.includes('welcome') || text.includes('about') || text.includes('intro')
    );
  });
  ...
  const titleElements = sectionElements.filter(el => {
    ...
    return (
      (tagName === 'h1' || tagName === 'h2' || tagName === 'h3') &&
      !isButtonLike && !isDescriptionTag &&
      (text.length > 5 && text.length < 100) &&
      !text.includes('get started') && !text.includes('book demo')
    );
  });
    ...
    switch (expectedType) {
      case 'title':
        return (tagName === 'h1' || tagName === 'h2' || tagName === 'h3') &&
          !text.includes('get started') && !text.includes('book demo');
      case 'description':
        return (tagName === 'p' || tagName === 'span') && text.length > 20 &&
          !text.includes('get started') && !text.includes('book demo') &&
          tagName !== 'h1' && tagName !== 'h2' && tagName !== 'h3';
      default:
        return true;
  ...
  const getInputBorderColor = (element, type) => {
    if (isElementSelected(element, type)) {
      return 'border-sprout-color-secondary';
    }
    return 'border-sprout-color-border-weak';
  };

  const getInputTextColor = (element, type) => {
    if (isElementSelected(element, type)) {
      return 'text-sprout-color-secondary';
    }
    return 'text-black';
  };
  ...
  const updateElementColor = (element, color) => {
    ...
    if (domElement) {
      ...
      domElement.className = updatedClasses;
  ...
  const toggleTheme = (theme) => {
    ...
    if (theme === 'dark') {
      ...
      // Apply dark theme to all editable elements
      const editableElements = container.querySelectorAll('.editable[data-key], .editable[data-store][data-field]');
      editableElements.forEach(el => {
        if (!el.classList.contains('text-[') && !el.classList.contains('text-white')) {
  ...
  const getAppliedThemeClasses = (container, theme) => {
    const themeClasses = {};
    const editableElements = container.querySelectorAll('.editable[data-key], .editable[data-store][data-field]');
    ...
    editableElements.forEach(el => {
      let identifier = '';
      if (el.dataset.key) {
        identifier = el.dataset.key;
      } else if (el.dataset.store && el.dataset.field) {
        identifier = `${el.dataset.store}.${el.dataset.field}`;
      }

      if (identifier) {
        themeClasses[identifier] = {
        ...
        <div className='pt-5 gap-8'>
          ...
          {(titleElements.length > 0 ? titleElements : [{ text: fallbackTitle, id: 'fallback-title' }]).map((element, index) => (
            <div key={`title-${element.id || index}`} className="mb-4">
              ...
              <div
                className={`relative mt-2 rounded-md border ${getInputBorderColor(element, 'title')} cursor-pointer`}
              ...
              >
                <Input
                  ...
                  className={`w-full border-none ${getInputTextColor(element, 'title')} px-3 md:px-4 py-3 text-sm md:text-base focus-visible:ring-0 focus:ring-0 focus:outline-none bg-transparent`}
                />
              </div>
            </div>
          ))}
          ...
          {(descriptionElements.length > 0 ? descriptionElements : [{ text: fallbackDescription, id: 'fallback-description' }]).map((element, index) => (
            <div key={`description-${element.id || index}`} className="mb-4">
              ...
              <div
                className={`relative mt-2 rounded-md border ${getInputBorderColor(element, 'description')} cursor-pointer`}
                onClick={() => handleElementClick(element, 'description')}
              >
                <textarea
                  value={element.text || ''}
                  onChange={(e) => {
                  ...
                  className={`w-full min-h-[100px] border-none ${getInputTextColor(element, 'description')} px-3 md:px-4 py-3 text-sm md:text-base focus-visible:ring-0 focus:ring-0 focus:outline-none bg-transparent resize-vertical`}
                />
              </div>
            </div>
          ))}
            ...
            <div className="flex flex-col gap-4 mt-4">
              <div className="flex justify-between p-2">
                ...
                <div className='border flex gap-2 border-sprout-color-border-weak rounded-md p-1'>
                  <Button
                    className={`border ${currentTheme === 'light'
                      ? 'border-sprout-color-secondary hover:border-sprout-color-secondary hover:bg-sprout-color-secondary-lightest bg-sprout-color-secondary-lightest text-sprout-color-secondary hover:text-sprout-color-secondary'
