...
const PageNode = ({ id, data }) => {
    ...
    const { boxOpen, setBoxOpen, activeSection, setActiveSection } = useBox();
    useEffect(() => {
                                            ...
                                            <Accordion type="single" collapsible className="w-full">
                                                ...
                                                >
                                                    <AccordionTrigger
                                                        ...
                                                        style={{ minWidth: "180", maxWidth: "180px" }}
                                                    >

                                                        <span className="truncate ml-2 flex-1 text-left"
                                                            onDoubleClickCapture={() => console.log("double clikec")}
                                                        >
                                                            {section.name}
                                                        </span>
                                                        ...
                                                        <button
                                                            className="absolute -bottom-3 right-4 z-30 px-1 py-1 text-gray-600 rounded-md  opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none group-hover:pointer-events-auto p-1 mt-1 bg-[#6E5AFF] text-white"
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                e.preventDefault();
...
const Hero = () => {
    ...
    const { boxOpen } = useBox();

    useEffect(() => {
        const allSections = nodes.flatMap(node =>
            Array.isArray(node.data?.sections)
                ? node.data.sections.map(section => ({
                    pageId: node.id,
                    pageTitle: node.data?.title,
                    sectionId: section.id,
                    sectionName: section.name,
                    isGlobal: globalLabels.includes(section.name)
                }))
                : []
        );

        const sectionNames = allSections.map(s => s.sectionName);

        if (JSON.stringify(sectionNames) !== JSON.stringify(updatedSectionNames)) {
            setUpdatedSectionNames(sectionNames);


            if (projectName && projectName !== 'Project Name') {
                updateSectionsInDB(sectionNames, projectName).catch(error => {
                    console.warn('Failed to update sections in DB:', error);
                });
            }
        }
    }, [nodes, projectName]);
    const updateSectionsInDB = async (sectionNames, projectName) => {
        ...
        try {
            ...
            console.log("Sections updated in DB:", response.data);
            return response.data;
        } catch (error) {
            console.error('Error updating sections in DB:', error);
            throw error;
        } finally {
    ...
    const handleInsertPaletteSection = async (paletteItem) => {
        ...
        if (projectName && projectName !== 'Project Name') {
            setTimeout(() => {
                const updatedSections = nodes.flatMap(node =>
                    Array.isArray(node.data?.sections)
                        ? node.data.sections.map(section => section.name)
                        : []
                );
                updateSectionsInDB(updatedSections, projectName).catch(error => {
                    console.warn('Failed to update sections in DB after insertion:', error);
                });
            }, 100);
        }
    };
    ...
    const generateSitemap = async (userPrompt) => {
        ...
        try {
            ...
            await updateSectionsInDB(allSectionNames, apiData.projectName);
    ...
    const saveSiteMap = async () => {
        ...
        try {
            ...
            // Also ensure sections are updated in the separate sections endpoint
            const currentSectionNames = nodesToSave.flatMap(node =>
                Array.isArray(node.data?.sections)
                    ? node.data.sections.map(section => section.name)
                    : []
            );

            await updateSectionsInDB(currentSectionNames, projectName);
    ...
    const globalSet = useMemo(() => new Set(globalLabels), [globalLabels]);
    ...
    // Add state to track the active section for making global
    const { setBoxOpen, activeSection, setActiveSection } = useBox();
    ...
    return (
        <div className="flex flex-col h-screen text-white">
            ...
            <div className="relative flex min-h-screen">
                ...
                {boxOpen && isSidebarOpen && (
                    <aside className="fixed left-20 top-3 bottom-0 w-80 bg-sprout-color-background-weaker shadow-xl roundedR-2xl z-50 p-4 overflow-y-auto">
                        ...
                        <div className="flex rounded-md p-1 mt-5 border bg-sprout-color-secondary-lightest border-sprout-color-secondary">
                            ...
                            <Button className="bg-transparent hover:bg-transparent cursor-pointer text-sprout-color-secondary" onClick={() => {

                                toggleGlobal(activeSection.sectionName)

                            }

                            }>Make a Global Section</Button>
                        </div>
                            ...
                            <div className='p-2 border flex gap-5 text-[#6F6765] border-sprout-color-border-weak rounded-md '>
                                ...
                                <div className='ml-16 mt-2'>
                                    ...
                                    <svg width="7" height="11" viewBox="0 0 7 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M1.17847 9.17847L5.17847 5.17847L1.17847 1.17847" stroke="#413735" strokeWidth="1.66667" strokeLinecap="square" strokeLinejoin="round" />
                                    </svg>
                        ...
                        "
                                    ...
                                    <TabsContent value="global">
                                        <div className='pt-5 p-3 pl-10 relative'>
                                            ...
                                            {isOpen && (
                                                <div className="absolute top-full left-3  bg-white border border-gray-200 rounded-md shadow-lg min-w-[220px] z-10">
